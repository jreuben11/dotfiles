...
# Add context usage with color based on percentage
if [ "$context_limit" -gt 0 ]; then
    # Choose color based on usage level
    if [ "$context_percent" -ge 75 ]; then
        context_color="${c_purple_fg}"  # High usage (75%+) - purple like the image
    ...
    # Format: ⛁ 78%
    status_line+=$(printf " ${context_color}⛁ ${context_percent}%%${c_reset}")
fi
